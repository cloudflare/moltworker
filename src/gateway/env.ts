...
export function buildEnvVars(env: MoltbotEnv): Record<string, string> {
  ...
  // Map MOLTBOT_GATEWAY_TOKEN to both OPENCLAW_GATEWAY_TOKEN and CLAWDBOT_GATEWAY_TOKEN
  // MOLTBOT_GATEWAY_TOKEN must be set to the node's device auth token
  // (from ~/.openclaw/identity/device-auth.json on the node host machine)
