 ...
 * - OPENROUTER_API_KEY: Your OpenRouter API key (preferred — use minimax/minimax-m2.5)
 *
 * Optional secrets:
...
app.all('*', async (c) => {
  ...
  try {
    ...
    if (!c.env.OPENROUTER_API_KEY && !c.env.ANTHROPIC_API_KEY && !c.env.OPENAI_API_KEY) {
      hint = 'No AI provider key set. Run: wrangler secret put OPENROUTER_API_KEY';
