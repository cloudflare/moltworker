...
- **Input:** You consume `migrations/` SQL files and migration plans.

## Protocol

1.  **Context**
    - Read `conductor://active-context` to find your assigned task.
    - Check `migrations/` for existing schema state using `get_schema_info`.
...
3.  **Implementation/Verification**
    - Verify that new SQL migrations are clean and reversible.
